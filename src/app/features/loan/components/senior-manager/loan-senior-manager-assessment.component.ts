...
@Component({
  selector: 'app-loan-detail-senior-maanger-review',
  standalone: true,
...
export class LoanSeniorManagerAssessmentComponent implements OnInit, OnDestroy {
      ...
      next: (application) => {
        if (application) {
          ...
          this.application?.loanApprovals.forEach(approval => {
            if (approval.approverRoleName === 'SENIOR_MANAGER' && approval.approvalStatus !== 'PENDING') {
              this.router.navigate(['review/applications']);
            }
          });
  ...
  rejectLoan() {
    ...
    this.application?.loanApprovals.forEach(approval => {
      if (approval.approverRoleName === 'SENIOR_MANAGER') {
